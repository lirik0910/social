...
use App\Http\Requests\Admin\ServicePage\CreateServicePageRequest;
...
class CreateServicePage
{
     ...
     * @param CreateServicePageRequest $args The arguments that were passed into the field.
     ...
     */
    protected function resolve($rootValue, CreateServicePageRequest $args, GraphQLContext $context, ResolveInfo $resolveInfo)
    {
        $user = $context->user();
        ...
        if (!isset($inputs['slug'])) {
            $slug = Str::slug(mb_strtolower($page->title), '-', $inputs['locale']);
        } else {
        ...
        if (!$page->save()) {
            throw new GraphQLSaveDataException(__('service_page.create_failed'), __('Error'));
        }
